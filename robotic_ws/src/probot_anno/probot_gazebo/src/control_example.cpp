...
int main(int argc, char **argv) {
    ...
    double end_pos[6] = {0.4, 0, 0.2,M_PI - M_PI/2, 0, 0};
    // Eigen::Matrix3d R_X;
    // Eigen::Matrix3d R_Y;
    // Eigen::Matrix3d R_Z;
    ...
    //         0, 0, 1;
    // R_end = R_Z*R_Y*R_X;
    // double w = 0.5*sqrt(1+R_end(0,0)+R_end(1,1)+R_end(2,2));
    // double x = (R_end(2,1)-R_end(1,2))/(4*w);
    // double y = (R_end(0,2)-R_end(2,0))/(4*w);
    // double z = (R_end(1,0)-R_end(0,1))/(4*w);

    geometry_msgs::Pose target_pose;
    target_pose.position.x = end_pos[0];
    target_pose.position.y = end_pos[1];
    target_pose.position.z = end_pos[2];
    target_pose.orientation.w = cos(end_pos[3]/2)*cos(end_pos[4]/2)*cos(end_pos[5]/2)+sin(end_pos[3]/2)*sin(end_pos[4]/2)*sin(end_pos[5]/2);
    target_pose.orientation.x = sin(end_pos[3]/2)*cos(end_pos[4]/2)*cos(end_pos[5]/2)-cos(end_pos[3]/2)*sin(end_pos[4]/2)*sin(end_pos[5]/2);
    target_pose.orientation.y = cos(end_pos[3]/2)*sin(end_pos[4]/2)*cos(end_pos[5]/2)+sin(end_pos[3]/2)*cos(end_pos[4]/2)*sin(end_pos[5]/2);
    target_pose.orientation.z = cos(end_pos[3]/2)*cos(end_pos[4]/2)*sin(end_pos[5]/2)-sin(end_pos[3]/2)*sin(end_pos[4]/2)*cos(end_pos[5]/2);
